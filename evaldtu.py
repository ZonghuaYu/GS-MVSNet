...
from load.evaldtu import LoadDataset
...
def eval(args):
    ...
    if args.pre_model is not None:
        checkpoint = torch.load(args.pre_model) #map_location=torch.device("cpu")  map_location=config.DEVICE
        model.load_state_dict(checkpoint["model"])  # strict=True

    model.to(config.DEVICE)

    # load dataset
    eval_dataset = LoadDataset(datasetpath=config.eval_root_dir, pairpath=config.eval_pair_path,
                               scencelist=config.eval_label, nviews=args.num_views)
    eval_databatch = DataLoader(eval_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.num_works,
...
if __name__ == "__main__":
    ...
    parser.add_argument('-w', '--num_works', default=1, type=int, help='dataset load works')
    parser.add_argument('-i', '--num_iters', default=5, type=int, help='number of iterations of the network')
    parser.add_argument('-v', '--num_views', default=5, type=int, help='views for input')
